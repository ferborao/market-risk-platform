...
def validate_ticker(ticker: str) -> None:
    ...
    if not required_columns.issubset(df.columns):
        print(f"Warning: Missing required columns in {ticker} data: {required_columns - set(df.columns)}")
    
    if df["Close"].isnull().any():
        print(f"Warning: Null values found in 'Close' column for ticker: {ticker}")
    
    if (df["Close"] <= 0.0).any():
        print(f"Warning: Values in 'Close' column for ticker: {ticker} are not positive")

    if df["Date"].isnull().any():
        print(f"Warning: Null values found in 'Date' column for ticker: {ticker}")
